...
function MagnifyingGlassIcon() {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" aria-hidden="true"><circle cx="10.7" cy="10.7" r="5.7" /><path d="m15.2 15.2 4.4 4.4" /><path d="M10.7 7.8v5.8M7.8 10.7h5.8" /></svg>;
}
...
export default function ProductGallery() {
  ...
  const openLightbox = (trigger: HTMLButtonElement, id = activeId) => {
  ...
  return <section className="product-gallery section" id="gallery" aria-labelledby="gallery-title">
    ...
    <div className="gallery-feature"><button className="gallery-screen gallery-enlarge" type="button" onClick={(event) => openLightbox(event.currentTarget)} aria-label={`Enlarge ${active.title} preview`}><img src={active.image} alt={active.alt} /><span className="gallery-zoom-cue" aria-hidden="true">↗ <small>Enlarge</small></span></button><div className="gallery-feature-copy"><p className="gallery-kicker">{active.eyebrow}</p><h3>{active.title}</h3><p>{active.description}</p><span className="gallery-note">Early-access preview · Interface still evolving</span></div></div>
    <div className="gallery-rail" aria-label="Kaizen OS product teaser views">{views.map((view) => <button key={view.id} className={`gallery-card ${activeId === view.id ? "is-active" : ""}`} type="button" aria-label={`Open enlarged ${view.eyebrow.replace(/^\d+ \/ /, "")} preview`} onClick={(event) => openLightbox(event.currentTarget, view.id)}><img src={view.image} alt="" /><span className="gallery-thumbnail-zoom" aria-hidden="true"><MagnifyingGlassIcon /></span><span><strong>{view.eyebrow}</strong><small>{view.id === "focus" ? "Daily focus" : view.id === "rhythm" ? "Weekly rhythm" : "Progress signals"}</small></span></button>)}</div>
